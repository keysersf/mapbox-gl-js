...
export class Placement {
    ...
    updateBucketOpacities(bucket: SymbolBucket, seenCrossTileIDs: { [string | number]: boolean }, collisionBoxArray: ?CollisionBoxArray) {
        ...
        // Push generated collision circles to buckets for debug rendering
        for (const bucketInstanceId in this.collisionCircleArrays) {
            const instance = this.collisionCircleArrays[bucketInstanceId];
            ...
            for (let i = 0; i < instance.circles.length; i += 4) {
                bucket.collisionCircleArray.emplaceBack(
                    ...
                    instance.circles[i + 2],
                    instance.circles[i + 3]
                );
            }
        }
